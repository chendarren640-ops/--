...
/************************************************************
 * Function :       LED_Toggle
 * Comment  :       用于切换LED端口的电平
 * Parameter:       LED_Port: LED端口号
 *                 LED_Pin: LED引脚号
 * Return   :       null
 * Author   :       Lingyu Meng
 * Date     :       2025-02-30 V0.1 original
************************************************************/
void LED_Toggle(uint32_t LED_Port , uint32_t LED_Pin)
{

	if (RESET == gpio_input_bit_get(LED_Port , LED_Pin))
	{
		gpio_bit_set(LED_Port , LED_Pin);     // 高电平   开灯
	}
	else
	{
		gpio_bit_reset(LED_Port , LED_Pin);   // 低电平   关灯
	}
