...
void LED_Init(void)
{
	...
	rcu_periph_clock_enable(RCU_GPIOB);    // 初始化GPIO_A总线时钟
	
	//初始化LED1端口
	gpio_mode_set(GPIOB, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, GPIO_PIN_4);   			// GPIO模式设置为输出
    gpio_output_options_set(GPIOB, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_4);   // 输出参数设置
	gpio_bit_reset(GPIOB, GPIO_PIN_4);  											// 引脚初始电平为低电平
