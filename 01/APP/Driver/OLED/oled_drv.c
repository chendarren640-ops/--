...
void OLED_ScrollDisplay(u8 num,u8 space)
{
	...
	while(1)
	{
		if(m==0)
		{
	    OLED_ShowChinese(128,8,t,16); //д��һ�����ֱ�����OLED_GRAM[][]������
			t++;
		}
		if(t==num)
			{
...
static u8 g_oled_page_base = 0;  /* 渲染基址: 0=下行(pages0-1), 2=上行(pages2-3) */

// 设置写入数据的起始位置 (128x32 OLED, 每行限2个page)
void OLED_WR_BP(u8 x,u8 y)
{
	u8 page = g_oled_page_base + y;
	if(page >= g_oled_page_base + 2) return;  /* 每行只写2页, 杜绝跨行覆盖 */
	OLED_WR_Byte(0xb0+page,OLED_CMD);//设置页起始地址
...
void OLED_ShowLine1(u8 *str){g_oled_page_base=2;OLED_ShowString(0,0,str,16);}  /* 上半屏 16px */
void OLED_ShowLine2(u8 *str){g_oled_page_base=0;OLED_ShowString(0,0,str,8);}   /* 下半屏 8px 永不越界 */
