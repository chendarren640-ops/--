...
int main(void)
{
    ...
    /* 静默 5 秒等待, 监听上位机中断命令 */
    {
        ...
        while (1) {
            ...
            if (elapsed >= NORMAL_BOOT_DELAY) break;
...
void JumpToApp(uint32_t app_addr)
{
    ...
    if ((app_stack & 0x2FFE0000UL) != 0x20000000UL ||
        app_entry < 0x08000000UL || app_entry > 0x08080000UL) {
        OLED_ShowLine2((uint8_t *)"NO APP!");
        OLED_Refresh();
        while (1) {
            gpio_bit_toggle(LED_SYS_PORT, LED_SYS_PIN);
